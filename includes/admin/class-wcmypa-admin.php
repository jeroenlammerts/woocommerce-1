...
class WCMYPA_Admin
{
    ...
    public function showOrderActions($order): void
    {
        ...
        $exportMode = WCMYPA()->setting_collection->getByName(WCMYPA_Settings::SETTING_EXPORT_MODE);
        $display    = 'display' === WCMYPA()->setting_collection->getByName(WCMYPA_Settings::SETTING_DOWNLOAD_DISPLAY);
        $attributes = [];
        ...
        if (WCMP_Settings_Data::EXPORT_MODE_PPS === $exportMode) {
            unset($listingActions[$getLabels]);
        }
        ...
        if (empty($processed_shipments) || 'NL' !== $shippingCountry) {
            unset($listingActions[$addReturn]);
        }

        foreach ($listingActions as $request => $data) {
            self::renderAction(
                $data['url'],
                $data['alt'],
                $data['img'],
